...
function sourceContext(label) {
  ...
  if (text.includes('guardian tiger')) return 'Tabletop exercises centered on Korea escalation, chemical/nuclear coercion, China intervention, and dual Taiwan-Korea crisis stress. Use this as the main source for alliance decision-cycle pressure and proportional-response gaps.';
...
function sourcesList(items) {
  return `<section class="section sources" id="sources"><h2>Source Index</h2><p class="section-lede">These are not generic bibliography links. Each card explains what analytical job the source performs in the page so readers can decide which original report to open first.</p><div class="source-grid">${items.map(([label, url]) => `<a class="source-card" href="${url}" target="_blank" rel="noopener noreferrer"><strong>${esc(label)}</strong><p>${esc(sourceContext(label))}</p><span>${esc(url)}</span></a>`).join('')}</div></section>`;
}
...
function comparisonMatrix(items, theater) {
  ...
  const warningText = theater === 'taiwan'
    ...
    : 'Watch missile dispersal, artillery readiness, SOF/drone activity, chemical or nuclear rhetoric, cyber disruption, Chinese positioning, Japan basing politics, and whether Taiwan crisis stress is pulling US assets away from Korea. The strongest evidence is convergence across military movement, public narrative, cyber disruption, and allied decision pressure.';
  return `<section class="section" id="comparison-matrix"><h2>Scenario Comparison Matrix</h2><p class="section-lede">The matrix is now presented as scenario cards because the useful comparison is not a spreadsheet cell. Each card shows the scenario's purpose, the warning problem, and the outcome logic a reader should carry forward to the watch page.</p><div class="visual-pair"><div class="analysis-card"><h3>Cross-Scenario Stress Profile</h3>${metrics.map(([label, value]) => `<div class="bar-row"><span>${esc(label)}</span><div class="bar-track"><i style="width:${value}%"></i></div><strong>${value}%</strong></div>`).join('')}<p>The percentages are qualitative stress scores for page triage, not statistical probabilities. They identify where analysts should look first when fresh reporting enters the cache.</p></div><div class="analysis-card"><h3>Warning Logic</h3><p>${esc(warningText)}</p></div></div><div class="comparison-grid" style="margin-top:18px">${items.map((s) => `<a class="matrix-card" href="${s.slug}.html"><img src="${esc(s.image)}" alt="${esc(s.caption)}" loading="lazy"><div class="scenario-body"><div class="matrix-meta">${esc(s.source)} // ${esc(s.family)}</div><h3>${esc(s.title)}</h3><p><strong>What it tests:</strong> ${esc(s.summary[0])}</p><p><strong>Outcome logic:</strong> ${esc(s.outcome)}</p><div class="card-actions"><span class="kicker">Read scenario page</span></div></div></a>`).join('')}</div></section>`;
}
...
function familiesSection(theater) {
  ...
  const koreaFamilies = [
    ['Limited Strike And Nuclear Coercion', '/assets/weapons/thaad-launcher.jpg', 'korea-scenario-guardian-tiger-limited-strike.html', 'This family tests how fast a limited conventional incident can move toward tactical nuclear threats. It is about the gap between conventional response and nuclear retaliation, and whether Pyongyang believes the alliance lacks proportional choices. The scenario is not primarily about North Korea winning a long war. It is about forcing negotiation under fear and uncertainty. The useful reader takeaway is that credible intermediate options, public messaging, and C2 discipline matter as much as raw firepower.'],
    ['Dual-Adversary Escalation', '/screenshots/final-taiwan-strait.png', 'korea-scenario-guardian-tiger-dual-contingency.html', 'This family links Korea to a Taiwan crisis. The key risk is that the United States and allies cannot deter opportunistic escalation by the second adversary while already managing the first. The page should therefore track Taiwan crisis intensity, Japan basing politics, missile-defense inventory, and DPRK launch tempo together. Treating Korea as isolated would hide the real risk. It is the scenario family most likely to expose munitions, ISR, tanker, cyber, and political-bandwidth shortages.'],
    ['Space, Cyber, And C2 Shock', '/assets/maps/korea-pusan-perimeter-usacmh.jpg', 'korea-scenario-nuclear-space.html', 'This family tests the information systems that allow modern forces and governments to operate. Satellite disruption, cyber outages, GPS degradation, public warning failure, and command-channel uncertainty can shape the first hours of a crisis. Korea is especially sensitive because distances are short and decisions must be made quickly. Resilience matters as much as firepower. The reader should watch for space-launch behavior, cyber preparation, communications backups, and public-warning continuity.'],
    ['Drone And Armor Attrition', '/assets/force-comparison/army-forces-china-vs-allies-taiwan/images/image_12.jpg', 'korea-scenario-drone-armor-attrition.html', 'This family tests whether armored and artillery forces can survive persistent observation and cheap precision attack. It converts lessons from Ukraine into Korean terrain, where concealment, movement, electronic warfare, counter-drone defense, and repair capacity matter. The warning indicators are practical: shelters, drone units, EW exercises, ammunition, and counter-UAS procurement. It also helps explain why apparently strong ground forces can lose operational value quickly if logistics and repair networks are visible.'],
    ['Nuclear-Cognitive Warfare', '/assets/maps/south-korea-physiography-cia-2018.jpg', 'korea-scenario-nuclear-cognitive-warfare.html', 'This family treats perception as a battlespace. North Korea can use nuclear narratives, cyber disruption, rumors, selective leaks, and public fear to slow allied decisions. It does not require immediate nuclear use to produce strategic effect. The page should therefore treat information operations as part of the threat model rather than background noise. The reader should watch for synchronized public fear, cyber outages, evacuation rumors, nuclear rhetoric, and wedge messaging aimed at Seoul and Washington.'],
  ];
  ...
  const analysis = theater === 'taiwan'
    ...
    : 'The map is not a decorative terrain panel. It is testing compressed escalation geography: Seoul and USFK nodes close to the DMZ, mountain and river corridors that constrain maneuver, southern ports that preserve logistics depth, and Japan/Guam access that turns a Korea crisis into a regional problem.';
  return `<section class="section" id="scenario-families"><h2>Scenario Families</h2><div class="map-feature">${figure(image, theater === 'taiwan' ? 'Taiwan terrain context for scenario families.' : 'Korean Peninsula terrain context for scenario families.')}<div class="analysis-card"><h3>What This Map Is Testing</h3><p>${esc(analysis)}</p><p>${theater === 'taiwan' ? 'Use this as the orientation layer before reading the cards: each scenario family is a different way of stressing the same geography, not a separate decorative storyline.' : 'Use this as the orientation layer before reading the cards: each scenario family stresses a different decision cycle, but all of them inherit the same distance, terrain, basing, and logistics constraints.'}</p></div></div><div class="family-grid family-grid-wide" style="margin-top:18px">${data.map(([title, img, href, text]) => `<a class="family-card" href="${href}"><img src="${esc(img)}" alt="${esc(title)}" loading="lazy"><h3>${esc(title)}</h3><p>${esc(text)}</p><div class="card-actions"><span class="kicker">Open deep review</span></div></a>`).join('')}</div></section>`;
}
...
function escalationSection(theater) {
  ...
  const images = theater === 'taiwan'
    ...
    : ['/assets/maps/north-korea-physiography-cia-2020.jpg', '/assets/weapons/thaad-launcher.jpg', '/screenshots/final-taiwan-strait.png'];
