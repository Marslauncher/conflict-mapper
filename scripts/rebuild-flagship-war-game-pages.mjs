...
function sourcesList(items, theater = 'all') {
...
function warningAndPlanning(theater) {
  if (theater === 'korea') {
    ...
    const planning = [
      ['Response Ladders', '/assets/weapons/thaad-launcher.jpg', 'Build response options between symbolic conventional strikes and excessive nuclear retaliation. Korea scenarios repeatedly punish a binary choice between doing too little and jumping too far up the escalation ladder. Useful planning includes pre-cleared proportional strike options, public-warning scripts, missile-defense allocation rules, civil-defense coordination, and explicit criteria for when cyber or space disruption becomes part of the military response. The objective is to deny Pyongyang the belief that nuclear threats create a usable gap in allied options.'],
      ['C2 And Public Resilience', '/assets/maps/south-korea-physiography-cia-2018.jpg', 'Harden command channels, public-warning systems, government communications, and continuity messaging against cyber and information shock. Seoul-area exposure means civilian confidence can affect military freedom of action in the first hours. The alliance should rehearse backup communications, emergency broadcast procedures, false-alert correction, transportation guidance, and local-government coordination. A resilient public information layer reduces the coercive value of rumors and nuclear narratives.'],
      ['Counter-Drone And Fires Integration', '/assets/weapons/prsm-missile.jpg', 'Treat drone defense, EW, counter-battery fires, camouflage, repair, and dispersed logistics as one system. Armor and artillery survivability depends on whether units can move, hide, refuel, repair, and communicate while under persistent observation. Planning should connect counter-UAS procurement with artillery displacement drills, hardened shelters, rapid runway repair, ammunition distribution, and long-range fires targeting. This is not a niche Ukraine lesson; it is central to Korea because roads, valleys, and bridges channel movement.'],
      ['Regional Coordination', '/assets/maps/korea-pusan-perimeter-usacmh.jpg', 'Institutionalize Korea-Japan-US crisis procedures for missile defense, basing, ISR, evacuation messaging, and munitions allocation. North Korea can exploit hesitation around external access or alliance consultation even if it lacks a winning conventional campaign. Planners should rehearse who speaks first, which bases matter, how interceptors are prioritized, and what public language denies wedge narratives. The Pusan logistics map is a reminder that operational depth matters only when political and command pathways stay open.'],
    ];
    return `<section class="section" id="warning-indicators"><h2>Warning Indicators</h2><p class="section-lede">The Korea warning model is built around watchable clusters. Concern rises when military posture, cyber or space disruption, nuclear messaging, public fear, and allied decision pressure move together; concern falls when events remain isolated, routine, or contradicted by calm logistics behavior.</p><div class="map-feature"><div class="analysis-card"><h3>Warning Cluster Dashboard</h3><div class="warning-grid">${warningRows.map(([title, text]) => `<div class="warning-item"><strong>${esc(title)}</strong><p>${esc(text)}</p></div>`).join('')}</div></div>${figure('/assets/maps/korea-pusan-perimeter-usacmh.jpg', 'Historical Korean War logistics map: ports, road corridors, and southern depth remain relevant if initial shock turns into sustained reinforcement.')}</div></section><section class="section" id="planning-implications"><h2>Planning Implications</h2><p class="section-lede">The practical implication of the Korea scenario set is that deterrence depends on response ladders, resilient command, counter-drone and fires integration, and regional coordination. The page should make those planning problems visible rather than leaving readers with a list of article titles.</p><div class="card-grid">${planning.map(([title, image, text]) => `<div class="analysis-card visual"><img src="${esc(image)}" alt="${esc(title)}" loading="lazy"><h3>${esc(title)}</h3><p>${esc(text)}</p></div>`).join('')}</div></section>`;
  }
...
${sourcesList(isTaiwan ? sources.taiwan : sources.korea, theater)}`;
