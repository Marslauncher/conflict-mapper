...
const MAX_ARTICLES_FOR_PROMPT = normalizeArticleLimit(process.env.SOURCE_BRIEF_ARTICLE_LIMIT, 500);  // Cap to avoid exceeding context limits
...
function gatherArticlesForPrompt(country = null, slug = null) {
  ...
  const filters = { timeRange: 24 }; // Prior 24 hours
  ...
  if (capped.length === 0) {
    return 'Prior-24-hour source brief: 0 RSS/news/website/think-tank feed articles were available. Use cautious language, identify feed coverage as a gap, and use live web search only to fill material blind spots.';
  }

  return buildLocalSourceBrief(capped);
}
...
function isPrior24HourArticle(article) {
  const timestamp = articleTimestamp(article);
...
function buildLocalSourceBrief(articles) {
  const sourceCounts = new Map();
  const categoryCounts = new Map();
  for (const article of articles) {
    const source = String(article.source || 'unknown source').toLowerCase();
    const category = String(article.category || 'general').toLowerCase();
    sourceCounts.set(source, (sourceCounts.get(source) || 0) + 1);
    categoryCounts.set(category, (categoryCounts.get(category) || 0) + 1);
  }
  return `Prior-24-hour source brief:
Corpus: ${articles.length} RSS/news/website/think-tank feed articles.
Source mix: ${formatCounts(sourceCounts)}
Topic mix: ${formatCounts(categoryCounts)}
Grounding rule: Base roughly 70% of the analysis on this sourced feed brief. Use roughly 30% additional at-time-of-prompt web/news search to eliminate blind spots, identify contradictions, and add missing context.

Source articles:

${articles.map(formatArticleForPrompt).join('\n\n---\n\n')}`;
}
...
The following prior-24-hour source brief summarizes monitored RSS, news, website, and think-tank feed articles:

${articles}

Based on this source brief and live web/news search at the time of prompting, produce the comprehensive global intelligence report in the exact JSON format specified. Weight the analysis about 70% toward the provided source brief and 30% toward additional web search to eliminate blind spots. Focus on identifying what has CHANGED from baseline normal activity — escalations, de-escalations, new developments, and emerging threats. Provide accurate geographic coordinates for all referenced locations.`;
...
Your output must be valid JSON matching this exact schema:
{
  ...
  "headerImage": {
    "url": "Direct URL to a relevant, high-quality news photo or satellite image related to today's top story",
  ...
  "sections": {
    "geopolitical": {
      ...
      "images": [{"url": "Direct image URL", "caption": "Description", "source": "Credit"}],
      "furtherReading": [{"title": "Article title", "url": "Article URL", "source": "Publication name"}],
      "mapCenter": {"lat": 0, "lng": 0, "zoom": 6, "label": "Key location for this section"}
    },
    "breaking": {
      "title": "Breaking News",
    ...
    "military": {
      ...
      "trendDirection": "ESCALATING|STABLE|IMPROVING|MIXED",
      "images": [{"url": "Direct image URL of weapon system or military asset", "caption": "Description", "source": "Credit"}],
      "furtherReading": [{"title": "Article title", "url": "Article URL", "source": "Publication name"}],
      "mapCenter": {"lat": 0, "lng": 0, "zoom": 6, "label": "Key military location"}
    },
    "political": {
      ...
      "keyDevelopments": ["Item 1", "Item 2", "Item 3"],
      "trendDirection": "ESCALATING|STABLE|IMPROVING|MIXED",
      "images": [{"url": "Direct image URL of leader or political event", "caption": "Description", "source": "Credit"}],
      "furtherReading": [{"title": "Article title", "url": "Article URL", "source": "Publication name"}]
    },
    "economic": {
      "title": "Economic News",
      ...
      "keyDevelopments": ["Item 1", "Item 2", "Item 3"],
      "trendDirection": "ESCALATING|STABLE|IMPROVING|MIXED",
      "images": [{"url": "Direct image URL of relevant chart, infrastructure, or economic visual", "caption": "Description", "source": "Credit"}],
      "furtherReading": [{"title": "Article title", "url": "Article URL", "source": "Publication name"}]
    },
    "science": {
    ...
    "robotics": {
      ...
      "keyDevelopments": ["Item 1", "Item 2", "Item 3"],
      "trendDirection": "ESCALATING|STABLE|IMPROVING|MIXED",
      "images": [{"url": "Direct image URL of drone or autonomous system", "caption": "Description", "source": "Credit"}],
      "furtherReading": [{"title": "Article title", "url": "Article URL", "source": "Publication name"}]
    }
  },
  "areasOfConcern": [
...
- For images: provide DIRECT image URLs (not article page URLs) — use news wire photos, satellite imagery, official government photos, or Reuters/AP/AFP images. Each section should have 1-3 relevant images where available. Omit the images array if no relevant image exists for that section.
- For furtherReading: provide 2-4 links per section to authoritative analysis pieces, news articles, or think tank reports. Use real, current URLs.
- For mapCenter: include only for sections where a map adds geographic context (military deployments, conflict zones, territorial disputes). Omit if not relevant.`;

const COUNTRY_USER_PROMPT = (countryName, articles) => `Current date: ${new Date().toUTCString()}
Country focus: ${countryName}

Prior-24-hour source brief from monitored RSS, news, website, and think-tank feeds, filtered for ${countryName} relevance:

${articles}

Produce the comprehensive ${countryName} intelligence dossier in the exact JSON format specified. Weight the analysis about 70% toward the provided source brief and 30% toward additional live web/news search at prompt time to eliminate blind spots. Highlight what is NEW or CHANGING versus the established baseline.`;
...
:root {
  ...
  --report-border: var(--border, rgba(255,255,255,0.08));
...
function renderCountryHTML(data) {
  ...
  // Render header image if present
  const headerImgHtml = data.headerImage?.url ? `
    <div style="margin-bottom:24px;border-radius:8px;overflow:hidden;border:1px solid rgba(255,255,255,0.08);">
      <img src="${data.headerImage.url}" alt="${data.headerImage.caption || ''}" style="width:100%;max-height:400px;object-fit:cover;display:block;" onerror="this.parentElement.style.display='none'">
      <div style="padding:8px 12px;background:#141820;font-family:'Share Tech Mono',monospace;font-size:10px;color:#4a5168;">${data.headerImage.caption || ''} ${data.headerImage.source ? '/ ' + data.headerImage.source : ''}</div>
    </div>` : '';
  ...
  const sectionsHtml = Object.entries(sections).map(([key, section]) => {
    ...
    const imagesHtml = (section.images || []).filter(img => img?.url).map(img => `
      <div style="border-radius:6px;overflow:hidden;border:1px solid rgba(255,255,255,0.06);">
        <img src="${img.url}" alt="${img.caption || ''}" loading="lazy" style="width:100%;max-height:280px;object-fit:contain;background:#0a0c10;display:block;" onerror="this.parentElement.style.display='none'">
        <div style="padding:6px 10px;background:#141820;font-family:'Share Tech Mono',monospace;font-size:9px;color:#4a5168;letter-spacing:0.5px;">${img.caption || ''} ${img.source ? '/ ' + img.source : ''}</div>
      </div>
    `).join('');

    // Render further reading links
    const readingHtml = (section.furtherReading || []).filter(r => r?.url).map(r => `
      <a href="${r.url}" target="_blank" style="display:flex;align-items:center;gap:8px;padding:6px 0;color:#6a9fd8;font-size:12px;text-decoration:none;border-bottom:1px solid rgba(255,255,255,0.04);">
        <span style="color:#00c8b4;font-size:10px;">→</span>
        <span style="flex:1;">${r.title || r.url}</span>
        <span style="font-family:'Share Tech Mono',monospace;font-size:9px;color:#4a5168;white-space:nowrap;">${r.source || ''}</span>
      </a>
    `).join('');

    // Render section mini-map if mapCenter provided
    const mapHtml = section.mapCenter?.lat ? `
      <div id="map-${key}" style="height:200px;border-radius:6px;border:1px solid rgba(255,255,255,0.06);margin-top:12px;"></div>
      <script>document.addEventListener('DOMContentLoaded',function(){var m=L.map('map-${key}',{center:[${section.mapCenter.lat},${section.mapCenter.lng}],zoom:${section.mapCenter.zoom||6},zoomControl:false,scrollWheelZoom:false,dragging:false,attributionControl:false});L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png',{subdomains:'abcd'}).addTo(m);L.circleMarker([${section.mapCenter.lat},${section.mapCenter.lng}],{radius:6,fillColor:'#00c8b4',color:'#fff',weight:2,fillOpacity:0.9}).addTo(m);L.tooltip({permanent:true,direction:'right',className:'map-label'}).setLatLng([${section.mapCenter.lat},${section.mapCenter.lng}]).setContent('${(section.mapCenter.label || '').replace(/'/g, "\\'")}'). addTo(m);});</script>
    ` : '';
    ...
    <div class="section" id="section-${key}">
      ...
      <div style="display:grid;grid-template-columns:1fr 280px;gap:20px;align-items:flex-start;">
        <div>
          ...
          ${imagesHtml ? `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:12px;margin-top:16px;">${imagesHtml}</div>` : ''}
          ${mapHtml}
          ${readingHtml ? `<div style="margin-top:16px;padding-top:12px;border-top:1px solid rgba(255,255,255,0.06);"><div style="font-family:'Share Tech Mono',monospace;font-size:9px;color:#00c8b4;letter-spacing:1px;margin-bottom:8px;">FURTHER READING</div>${readingHtml}</div>` : ''}
        </div>
        ${devItems ? `
...
.map-label { background:none!important;border:none!important;box-shadow:none!important;font-family:'Share Tech Mono',monospace;font-size:10px;color:#00c8b4;letter-spacing:0.5px; }
</style>
