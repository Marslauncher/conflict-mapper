...
async function generateAllCountries(options = {}, progressCallback = null) {
  ...
  for (let i = 0; i < COUNTRY_SLUGS.length; i++) {
    ...
    if (progressCallback) {
      progressCallback({ current: i + 1, total: COUNTRY_SLUGS.length, slug, done: false });
    }

    const result = await generateCountryAnalysis(slug, options);
    ...
    if (progressCallback) {
      progressCallback({ current: i + 1, total: COUNTRY_SLUGS.length, slug, done: true, success: result.success });
    }

    // Brief pause between requests to avoid rate limiting
    if (i < COUNTRY_SLUGS.length - 1) {
...
async function generateAll(options = {}, progressCallback = null) {
  if (progressCallback) {
    progressCallback({ phase: 'global', current: 0, total: COUNTRY_SLUGS.length + 1, message: 'Generating global report...' });
  }

  const globalResult = await generateGlobalAnalysis(options);

  const countryResults = await generateAllCountries(options, (update) => {
    if (progressCallback) {
      progressCallback({
        phase:   'countries',
        ...
        slug:    update.slug,
        message: `Generating ${update.slug} report... (${update.current}/${COUNTRY_SLUGS.length})`,
      });
    }
  });
