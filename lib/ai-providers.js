...
async function httpPost(url, headers, body) {
  ...
  try {
    ...
    throw new Error(`HTTP ${response.status}: non-JSON response from ${url}: ${text.slice(0, 180)}`);
  }

  if (!response.ok) {
    const message = data?.error?.message || data?.error || data?.message || text || 'Unknown error';
...
// ─────────────────────────────────────────────────────────────────────────────
// PROVIDER: PERPLEXITY
// Uses the current Sonar API with online search capability.
// ─────────────────────────────────────────────────────────────────────────────
...
class PerplexityProvider {
  ...
  async chat(systemPrompt, userPrompt, options = {}) {
    ...
    const isDeepResearch = this.model.includes('deep-research');
    ...
    // sonar-deep-research does not support max_tokens — it generates
    // as much as needed. Other models get a token limit.
    if (!isDeepResearch) {
      body.max_tokens = options.maxTokens || 16000;
    }

    // sonar-deep-research does not support temperature
    if (!isDeepResearch) {
      body.temperature = options.temperature !== undefined ? options.temperature : 0.3;
    }

    console.log(`[perplexity] Calling ${this.model}${isDeepResearch ? ' (deep research — no token limit)' : ` (max_tokens: ${body.max_tokens})`}`);

    const data = await httpPost(
      buildPerplexityUrl(this.baseUrl),
    ...
    console.log(`[perplexity] Response: ${content.length} chars, finish_reason: ${finishReason}`);
...
// ─────────────────────────────────────────────────────────────────────────────
// PROVIDER: OPENROUTER
// OpenAI-compatible model router. Model IDs are provider/model strings.
// ─────────────────────────────────────────────────────────────────────────────
...
class OpenRouterProvider extends OpenAIProvider {
  constructor(config) {
    ...
    this.siteUrl = config.siteUrl || config.referer || '';
    this.appTitle = config.appTitle || 'Conflict Mapper';
  }
 ...
 * @returns {PerplexityProvider|OpenRouterProvider|OpenAIProvider|AnthropicProvider|GoogleProvider|LocalProvider}
 */
...
function listProviders() {
  return [
    ...
    { id: 'openrouter', name: 'OpenRouter',      defaultModel: 'anthropic/claude-sonnet-4',    note: 'Routes to many hosted models through one API' },
