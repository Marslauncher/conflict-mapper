...
function saveArticles(newArticles) {
  ...
  for (const article of newArticles) {
    ...
    if (nt && titleMap.has(nt)) {
      // Keep the more useful representative for this story.
      const existing_match = titleMap.get(nt);
      const newDate  = articleTimestamp(article);
      const existDate = articleTimestamp(existing_match);
      if (articleContextPriority(article) > articleContextPriority(existing_match)
        || (articleContextPriority(article) === articleContextPriority(existing_match) && newDate > existDate)) {
  ...
  merged.sort((a, b) => articleTimestamp(b) - articleTimestamp(a));
...
function articleContextPriority(article) {
  ...
  let score = Math.max(0, 48 - Math.min(ageHours, 48)) + (categoryWeights[category] || 6);
  if (/(missile|drone|airstrike|warship|troops?|nuclear|sanctions?|cyberattack|ransomware|critical infrastructure|shipping lane|taiwan strait|south china sea|red sea|black sea|hormuz|gaza|iran|israel|ukraine|russia|china|north korea|nato|semiconductor|export control)/.test(text)) score += 18;
  if (/(reuters|associated press|ap news|bbc|financial times|new york times|bloomberg|foreign policy|war on the rocks|csis|rusi|rand|iiss|atlantic council|usni|breaking defense|defense news|defence|radio free europe)/.test(source)) score += 8;
...
function normalizeSettings(input = {}) {
  ...
  const rawMinutes = Number(
    input.feedRefreshIntervalMinutes
      ...
      ?? (Number.isFinite(legacyHours) ? legacyHours * 60 : DEFAULT_SETTINGS.feedRefreshIntervalMinutes)
  );
  const feedRefreshIntervalMinutes = Number.isFinite(rawMinutes)
    ? Math.min(1440, Math.max(1, Math.round(rawMinutes)))
    : DEFAULT_SETTINGS.feedRefreshIntervalMinutes;
  const maxArticlesPerFeed = boundedNumber(input.maxArticlesPerFeed, DEFAULT_SETTINGS.maxArticlesPerFeed, 1, 200);
  const deduplicationThreshold = boundedNumber(input.deduplicationThreshold, DEFAULT_SETTINGS.deduplicationThreshold, 0.5, 1);
  const reportRetentionDays = boundedNumber(input.reportRetentionDays, DEFAULT_SETTINGS.reportRetentionDays, 1, 365);
  const defaultCountries = Array.isArray(input.defaultCountries) && input.defaultCountries.length
    ...
    : DEFAULT_SETTINGS.defaultCountries;
