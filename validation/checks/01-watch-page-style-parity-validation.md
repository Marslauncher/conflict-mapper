...
- No legacy watch-page `.topbar` / breadcrumb top menu is rendered. Navigation must be handled by the primary site navigation pattern, not an extra page-specific top menu.
...
- Citation overlays render above maps, expanded map controls, Leaflet controls, cards, and sidebars. Validate with `elementsFromPoint` or an equivalent rendered check, not DOM presence alone.
